...
function calculateConfidenceScore(prediction) {
    ...
    return {
        overall: weightedAverage([
            { value: historicalAccuracy, weight: 0.4 },
            { value: dataQuality, weight: 0.2 },
            { value: modelConfidence, weight: 0.3 },
            { value: variableStability, weight: 0.1 }
        ]),
        factors: {
            historicalAccuracy,
            dataQuality,
...
function generatePredictionInsights(prediction) {
    return {
        keyFactors: analyzeKeyFactors(prediction),
        confidenceAnalysis: analyzeConfidenceFactors(prediction),
        historicalContext: provideHistoricalContext(prediction),
        uncertaintyFactors: identifyUncertaintyFactors(prediction),
        recommendations: generateRecommendations(prediction)
    };
}

function generateAlternativeScenarios(prediction) {
    return {
        optimistic: calculateScenario(prediction, 'optimistic'),
        pessimistic: calculateScenario(prediction, 'pessimistic'),
        neutral: calculateScenario(prediction, 'neutral'),
        variations: generateVariations(prediction)
    };
}

function analyzeFactorContributions(result, weights) {
    return {
        individual: calculateIndividualContributions(result, weights),
        interactions: analyzeFactorInteractions(result),
        sensitivity: performSensitivityAnalysis(result),
        importance: rankFactorImportance(result)
    };
}

function calculateMultiFactorConfidence(result, weights) {
    return {
        overall: computeOverallConfidence(result, weights),
        byFactor: computeFactorConfidences(result),
        interactions: assessFactorInteractions(result),
        reliability: assessReliabilityScore(result)
    };
}

function weightedAverage(items) {
    const sum = items.reduce((acc, { value, weight }) => acc + (value * weight), 0);
    const weightSum = items.reduce((acc, { weight }) => acc + weight, 0);
...
function calculateConfidenceScore(prediction) {
    return {
        ...
        factors: {
            historicalAccuracy: prediction.historicalAccuracy || 0,
            dataQuality: prediction.dataQuality || 0,
            modelConfidence: prediction.modelConfidence || 0,
            variableStability: prediction.variableStability || 0
        }
    };
}
...
function generatePredictionInsights(prediction) {
    return {
        keyFactors: analyzeKeyFactors(prediction),
        confidence: analyzeConfidenceFactors(prediction),
        historicalContext: getHistoricalContext(prediction),
        recommendations: generateRecommendations(prediction)
    };
}

function analyzeKeyFactors(prediction) {
...
function analyzeConfidenceFactors(prediction) {
    return {
        overallConfidence: prediction.confidence,
        confidenceByFactor: prediction.factorConfidence || {},
        reliabilityScore: prediction.reliability || 0,
        uncertaintyFactors: prediction.uncertaintyFactors || []
    };
}

function getHistoricalContext(prediction) {
    return {
        similarPredictions: prediction.similarHistoricalPredictions || [],
        accuracy: prediction.historicalAccuracy || 0,
        reliability: prediction.historicalReliability || 0,
        trends: prediction.historicalTrends || []
    };
}
...
function generateRecommendations(prediction) {
    return {
        ...
        alternativeScenarios: prediction.alternatives || []
    };
}
